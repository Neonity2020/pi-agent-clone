...
export class MiniMaxTransport implements ProviderTransport {
  ...
  async *stream(options: StreamOptions): AsyncGenerator<StreamEvent> {
    ...
    if (!key) {
      yield { type: "error", error: new Error("MINIMAX_API_KEY not set"), message: emptyAssistantMessage() };
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        yield { type: "error", error: new Error(`MiniMax API error ${response.status}: ${text}`), message: partial };
        return;
      }

      if (!response.body) {
        yield { type: "error", error: new Error("MiniMax: empty response body"), message: partial };
        return;
      }
      ...
      if (partial.toolCalls) {
        for (let i = 0; i < partial.toolCalls.length; i++) {
          if (!partial.toolCalls[i].id) {
            partial.toolCalls[i].id = `mm_call_${i}_${Date.now()}`;
          }
        }
      }
